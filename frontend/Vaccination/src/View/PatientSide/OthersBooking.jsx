...
const OthersBooking = () => {
  ...
  useEffect(() => {
    if (userSession.user?.email) {
      axios.get(`http://localhost:3000/vaccinelistothers/${userSession.user.email}`)
        .then(res => {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ flex: '1', padding: '20px', boxSizing: 'border-box' }}>
          <h2 style={{ textAlign: 'center', marginBottom: '20px' }}>Others Vaccination History</h2>
              ...
              <tbody>
                {vaccines.length ? vaccines.map((vaccine, i) => (
                  <tr key={i} style={{ textAlign: 'center' }}>
                    <td style={tdStyle}>{vaccine.fullname}</td>
                    <td style={tdStyle}>{vaccine.nic}</td>
