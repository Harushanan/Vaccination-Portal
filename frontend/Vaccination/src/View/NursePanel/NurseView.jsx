...
const NurseView = () => {
  ...
  const [book, setBook] = useState([]); // assuming book is an array of bookings
  const [user, setUser] = useState({});

  useEffect(() => {
    axios.get(`http://localhost:3000/nursevaccinepersonlist/${id}`)
      .then(res => {
        if (res.data.message === "Fetch successful") {
          setVaccines(res.data.getvaccine || {});
          setUser(res.data.getuser || {});
          setBook(res.data.mybook || []); 
        }
      })
      .catch(console.error);
  }, [id]);
  ...
  const handleDownloadPdf = () => {
    ...
    html2canvas(reportRef.current, { scale: 2, useCORS: true }).then((canvas) => {
      ...
      pdf.save(`${user.email || 'report'}.pdf`);
    });
  };
          ...
          <div className="info-section">
            ...
            <div className="info-right">
              <div className="text-block">
                ...
                <p><strong>Patient ID:</strong> {user._id || '-'}</p>
              </div>
          ...
          <table>
            ...
            <tbody>
              {Array.isArray(book) && book.length > 0 ? (
                book
                  .filter(b => b.vaccine === vaccines.Name)
                  .map((b, i) => (
                    <tr key={i}>
                      <td>{b.vaccine}</td>
                      <td>{b.dose}</td>
                      ...
                      <td>{b.center}</td>
                      <td>
                        <span className={`status-tag ${b.status?.toLowerCase() || 'default'}`}>
                          {b.status}
                        </span>
                      </td>
                      <td>{b.removereason === 'no reason provided' ? '-' : b.removereason || '-'}</td>
                      <td>{b.status === 'inject' ? `${b.injectBy || '-'} (${b.injectById || '-'})` : '-'}</td>
                    </tr>
                  ))
              ) : (
                <tr>
